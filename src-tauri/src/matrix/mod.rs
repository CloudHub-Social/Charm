...
pub struct MatrixState {
    ...
    /// One room-scoped cancellation flag per in-flight `send_attachment` call,
    /// keyed by the upload's `txn_id`. `send::cancel_attachment_upload` flips the flag;
    ...
    pub(crate) attachment_cancellations: std::sync::Mutex<
    ...
    /// Taken after login exclusion; opt-out/logout cannot race a token refresh.
    pub(crate) push_lifecycle_lock: Mutex<()>,
...
impl Default for MatrixState {
    fn default() -> Self {
        Self {
            ...
            push_lifecycle_lock: Mutex::default(),
