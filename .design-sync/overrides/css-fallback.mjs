...
export function fallbackCssFromStorybook({ bundleCss, sbStatic, out }) {
  ...
  if (links[0]) {
    ...
    // [ds-sync fork — Charm] The generated preview-card chrome hard-codes a
    // white body (emit.mjs), which hides Charm's light-on-transparent content
    // (input labels, ghost/link buttons, disabled/placeholder text) since Charm
    // is dark-first. Paint the CARD-ONLY containers (.ds-grid/.ds-cell/.ds-single
    // exist only in picker cards, never in rendered designs) with the design
    // system canvas token so cards match the storybook reference. Real designs —
    // which receive only styles.css's @import closure — are untouched.
    const cardCanvas =
      '\n/* [ds-sync fork] dark-first picker-card canvas — see .design-sync/overrides/css-fallback.mjs */\n'
      + '.ds-grid,.ds-cell,.ds-single{background-color:var(--color-bg-base)}\n';
    writeFileSync(bundleCss, css + cardCanvas);
