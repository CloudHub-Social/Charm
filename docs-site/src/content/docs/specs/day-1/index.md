...
| 39 | [Timeline state and membership events](/specs/day-1/spec-39--timeline-state-and-membership-events/) | **In progress** | Full desktop/web stream, notices, collapse, controls, tests, and gallery are in stacked review; merge and real-room evidence remain |
...
| 45 | [Registration and password reset](/specs/day-1/spec-45--registration-and-password-reset-flows/) | **In progress** | Desktop registration UIA, provider-aware SSO, and advertised token login; web, password reset, provider icons, and live verification remain |
...
| 63 | [Sidebar and space management](/specs/day-1/spec-63--sidebar-and-space-management-pin-reorder-context-menu-add-existing/) | **In progress** | Pin/unpin, cross-device-synced reorder, Leave, Remove, Set/Unset Suggested, Add Existing, and power-level gating on `SpaceRail`'s context menu all implemented; `RoomList`'s hierarchy-row `Remove from space` is intentionally still ungated (follow-up), and `Settings` is deferred to Spec 33's not-yet-built space-settings surface |
