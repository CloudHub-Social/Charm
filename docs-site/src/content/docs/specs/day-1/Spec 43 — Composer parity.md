...
already-authored formatting. If the caret is in the middle of a code block, the
kill switch inserts a plain paragraph after it and moves the selection there rather
than relying on the end-of-block-only exit command. Strike/code-block keyboard shortcuts, input rules,
