...
- **Notifications**: Rust owns the platform notification boundary. Desktop and Android use
  ...
  `is_notification_permission_granted` Tauri commands. On iOS, Rust re-reads the
  native token for an already-enabled registration after session restoration and
  every foreground; no token event or registration command is exposed to the
  renderer. Local notifications are triggered
